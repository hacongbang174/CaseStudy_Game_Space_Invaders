...
let animate = function () {
    ...
    grid.invader.forEach((invader, i) => {
      ...
      projectiles.forEach((projectile, j) => {
        if (
          ...
          setTimeout(() => {
            let invaderFound = grid.invader.find(
            ...
            if (invaderFound && projectileFound) {
              ...
              if (score > 5000) {
                for (let i = 0; i < grids.length; i++) grids[i].velocity.x = 5;
              } else if (score > 10000) {
                for (let i = 0; i < grids.length; i++) grids[i].velocity.x = 10;
              } else if (score > 20000) {
                for (let i = 0; i < grids.length; i++) grids[i].velocity.x = 15;
              }
            }
          }, 0);
        }
  ...
  if (ButtonState.BUTTON_LEFT.pressed && player.position.x >= 0) {
    ...
    ButtonState.BUTTON_RIGHT.pressed &&
    player.position.x + player.width <= canvas.width
  ) {
    player.velocity.x = 7;
    player.rotation = 0.15;
  } else if (
    ButtonState.BUTTON_UP.pressed &&
